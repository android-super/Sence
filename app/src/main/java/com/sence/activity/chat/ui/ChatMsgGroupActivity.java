...
import com.sence.bean.request.chat.RSendImgMessageBean;
import com.sence.bean.request.chat.RSendMessageBean;
...
public class ChatMsgGroupActivity extends BaseActivity implements View.OnClickListener {
            ...
            public void onItemChildClick(BaseQuickAdapter ba, View view, int position) {
                switch (view.getId()) {
                    case R.id.head_left://左边头像
                        Intent intent = new Intent(ChatMsgGroupActivity.this, MyInfoActivity.class);
                        intent.putExtra("uid", reAdapter.getData().get(position).getUid());
                        startActivity(intent);
                        break;
                    case R.id.head_right://右边头像 自己
                        intent = new Intent(ChatMsgGroupActivity.this, MyInfoActivity.class);
                        intent.putExtra("uid", reAdapter.getData().get(position).getUid());
                        startActivity(intent);
                        break;
                    ...
                    case R.id.right_image://右边大图片
                        if (reAdapter.getData().get(position).getType() == 2) {
                            ...
                            startActivity(new Intent(ChatMsgGroupActivity.this, FullImageActivity.class));
    ...
    public int onActLayout() {
        return R.layout.activity_chat_msg_group;
    }
    ...
    private void readMsgData() {
        HttpManager.getInstance().PlayNetCode(HttpCode.CHAT_READ_GROUP, new RVidBean(LoginStatus.getUid(), v_id)).request(new ApiCallBack() {
            @Override
            public void onFinish() {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.chat_function_photo://选择照片
                PictureSelector.create(ChatMsgGroupActivity.this)
